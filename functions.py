...
def replace_gas_keys_names(gas_prices):
    key_patterns = {
        GASOLINE_95: re.compile(GASOLINE_95_PATTERN, re.IGNORECASE),
        DIESEL: re.compile(DIESEL_PATTERN, re.IGNORECASE),
        COLORED_DIESEL: re.compile(COLORED_DIESEL_PATTERN, re.IGNORECASE),
    }
...
# Format one history CSV row from the current week's gas data
def csv_row(start_date, end_date, gas_data, pdf_url):
    p95 = gas_data.get(GASOLINE_95, "")
    diesel = gas_data.get(DIESEL, "")
    colored = gas_data.get(COLORED_DIESEL, "")
    p98 = gas_data.get(GASOLINE_98, "")
    return f"{start_date},{end_date},{p95},{diesel},{colored},{p98},{pdf_url}\n"

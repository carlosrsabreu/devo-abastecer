...
span_with_gas_info = s.find(
    'span', {'id': 'dnn_ctr9883_View_D1_dlstInformacaoOne_Conteudo_0'})
...
while i < len(gas_info) - 1:
    ...
    if add_date:
        try:
            start_date, end_date = gas_info[i].replace(
                '.', '-').split('\xa0a ')
        except Exception:
        ...
        dict_prices[last_id] = {
            START_DATE_KEY: start_date, END_DATE_KEY: end_date, GAS_KEY: []}
        i += 1
    # Gas information
    else:
        gas = gas_info[i]
        price = float(gas_info[i + 1][:-1].replace(',', '.'))
        dict_prices[last_id][GAS_KEY].append(
            {GAS_NAME_KEY: gas, GAS_PRICE_KEY: price})
        i += 2
